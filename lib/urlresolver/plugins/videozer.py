...
class VideozerResolver(Plugin, UrlResolver, PluginSettings):
    ...
    def get_media_url(self, host, media_id):
        ...
        try:
            ...
            spn_ik = unhexlify(
                self.__decrypt(aData["cfg"]["login"]["spen"], aData["cfg"]["login"]["salt"], 950569)).split(';')
            spn = spn_ik[0].split('&')
            ik = spn_ik[1]
            ...
            for item in spn:
                item = item.split('=')
                if int(item[1]) == 1:
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["info"]["sece2"],
                                                                   aData["cfg"]["environment"]["rkts"],
                                                                   key) + '&' #decrypt32byte
                elif int(item[1] == 2):
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["ads"]["g_ads"]["url"],
                                                                   aData["cfg"]["environment"]["rkts"], key) + '&'
                elif int(item[1]) == 3:
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["ads"]["g_ads"]["type"],
                                                                   aData["cfg"]["environment"]["rkts"], key, 26, 25431,
                                                                   56989, 93, 32589, 784152) + '&'
                elif int(item[1]) == 4:
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["ads"]["g_ads"]["time"],
                                                                   aData["cfg"]["environment"]["rkts"], key, 82, 84669,
                                                                   48779, 32, 65598, 115498) + '&'
                elif int(item[1]) == 5:
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["login"]["euno"],
                                                                   aData["cfg"]["login"]["pepper"], key, 10, 12254,
                                                                   95369, 39, 21544, 545555) + '&'
                elif int(item[1]) == 6:
                    sLink = sLink + item[0] + '=' + self.__decrypt(aData["cfg"]["login"]["sugar"],
                                                                   aData["cfg"]["ads"]["lightbox2"]["time"], key, 22,
