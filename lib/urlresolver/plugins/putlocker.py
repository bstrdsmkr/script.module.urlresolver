...
class PutlockerResolver(Plugin, UrlResolver, PluginSettings):
    ...
    def get_media_url(self, host, media_id):
        ...
        try:
            ...
            common.addon.log_error('putlocker: got http error %d fetching %s' % (e.code, web_url))
        ...
        if r:
            ...
            msg = '%s: session hash not found' % self.name
        ...
        if q == '1':
            ...
            if 'putlocker' in host:
                ...
                return "%s|User-Agent=%s" % (
                    final, 'Mozilla%2F5.0%20(Windows%20NT%206.1%3B%20rv%3A11.0)%20Gecko%2F20100101%20Firefox%2F11.0')
            else:
        ...
        #Else grab standard flv link
            ...
            try:
                ...
                common.addon.log_error('putlocker: got http error %d fetching %s'(e.code, xml_url))
                return self.unresolvable(3, str())
            ...
            if r:
                ...
                return self.unresolvable(0, 'Stream url not found')
    ...
    def valid_url(self, url, host):
        ...
        return (re.match('http://(www.)?(putlocker|sockshare).com/' + '(file|embed)/[0-9A-Z]+',
                         url) or 'putlocker' in host or 'sockshare' in host)
    ...
    def login(self):
        if self.login_stale():
            ...
            if solution:
                ...
                data = {'user': self.get_setting('username'), 'pass': self.get_setting('password'),
                        'captcha_code': solution, 'remember': 1, 'login_submit': 'Login'}
