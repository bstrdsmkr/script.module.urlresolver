...
class VideobbResolver(Plugin, UrlResolver, PluginSettings):
    ...
    def get_media_url(self, host, media_id):
        ...
        if r:
            for match in r:
                ...
                res = int(res.strip('p'))
                if chosen_res < res <= max_res:
                    stream_url_part1 = url.decode('base-64')
                    chosen_res = res
        else:
            msg = 'videobb: stream url part1 not found'
        ...
        spn_ik = unhexlify(
            self.__decrypt(aData["settings"]["login_status"]["spen"], aData["settings"]["login_status"]["salt"],
                           950569)).split(';')
        spn = spn_ik[0].split('&')
        ik = spn_ik[1]
        ...
        for item in spn:
            item = item.split('=')
            if int(item[1]) == 1:
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["info"]["sece2"],
                                                               aData["settings"]["config"]["rkts"],
                                                               key) + '&' #decrypt32byte
            elif int(item[1] == 2):
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["banner"]["g_ads"]["url"],
                                                               aData["settings"]["config"]["rkts"], key) + '&'
            elif int(item[1]) == 3:
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["banner"]["g_ads"]["type"],
                                                               aData["settings"]["config"]["rkts"], key, 26, 25431,
                                                               56989, 93, 32589, 784152) + '&'
            elif int(item[1]) == 4:
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["banner"]["g_ads"]["time"],
                                                               aData["settings"]["config"]["rkts"], key, 82, 84669,
                                                               48779, 32, 65598, 115498) + '&'
            elif int(item[1]) == 5:
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["login_status"]["euno"],
                                                               aData["settings"]["login_status"]["pepper"], key, 10,
                                                               12254, 95369, 39, 21544, 545555) + '&'
            elif int(item[1]) == 6:
                sLink = sLink + item[0] + '=' + self.__decrypt(aData["settings"]["login_status"]["sugar"],
                                                               aData["settings"]["banner"]["lightbox2"]["time"], key,
                                                               22, 66595, 17447, 52, 66852, 400595) + '&'
    ...
    def __decrypt(self, text, k1, k2, p4=11, p5=77213, p6=81371, p7=17, p8=92717, p9=192811):
        tobin = self.hex2bin(text, len(text) * 4)
